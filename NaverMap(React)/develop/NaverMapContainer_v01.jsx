...
const NaverMapContainer = (props) => {
            ...
            .then((res) => {
                ...
                for(let i=0; i<datalist.length; i++){
                    ...
                    if(props.clickData !== undefined && Object.keys(props.clickData).length > 0){
                        ...
                        if(startNodeMarker.current !== null && startNodeMarker.current.nodeId === marker.nodeId){
                            marker.setIcon(pointImage);
                            marker.setMap(map.current);
                            markerArr.push(marker);
                        }else if(endNodeMarker.current !== null && endNodeMarker.current.nodeId === marker.nodeId){
                            marker.setIcon(pointImage);
                            marker.setMap(map.current);
                            markerArr.push(marker);
                        }else{
            ...
            .then((res) => {
                ...
                if(polyLineArray.current.length > 0){
                    for(let i=0; i<polyLineArray.current.length; i++){
                        polyLineArray.current[i].setMap(map.current);
                    }
                }
    ...
    useEffect(() => {
        console.log("minMaxEvt Effect 실행");
    ...
    // 수집구간 상세 폴리라인 표출 함수
    useEffect(() => {
        let viewDatalist = props.mapConf.mapDetail?.viewDatalist
        let mode = props.mode?.mode;
        ...
        if(viewDatalist !== undefined && viewDatalist.length > 0){
            ...
            for(let i=0; i<Object.keys(linkObj).length; i++){
                ...
                if(mode === "C" || mode === "U"){
                    //폴리라인 클릭 이벤트
                    navermaps.Event.addListener(polyLineArr[i], 'click', function (e) {
                        if(e.overlay.getOptions("strokeColor") === "#808080"){
                            //선택한 경우
                            e.overlay.setOptions("strokeColor", "#FF0000");
                            polyLineArray.current.push(e.overlay);
    ...
    }, [props.mapConf.mapDetail?.viewDatalist])

    /******************** map, gridMap 관련 useEffect ********************/

    // 마커 표출 관련 Effect
    useEffect(() => {
        if(props.mapConf.fnMarker?.data.length > 0){
            // 지도 위의 마커 초기화
            const  markerArrayLength = markerArray.current.length;
    ...
    useEffect(() => {
        if(props.mapConf.fnMarkerEvt?.type !== null){
            ...
            //이벤트 타입이 애니메이션인 경우
            if(markerEvtType === "Animation"){
                if(beforeMarker.current !== null){
                    console.log(beforeMarker.current);
                ...
                for(let i=0; i<markerArray.current.length; i++){
                    if(markerArray.current[i][markerKey] === markerEvtData[markerKey]){
                        beforeMarker.current = markerArray.current[i];
                        markerArray.current[i].setAnimation(naver.maps.Animation.BOUNCE);
                        map.current.setCenter(new navermaps.LatLng(markerArray.current[i].getPosition().y, markerArray.current[i].getPosition().x));
    ...
    useEffect(() => {
        if(props.clickData.length !== 0 && props.clickData !== undefined){
            ...
            // if(props.mapConf.fnMarker !== undefined){
            //     if(beforeMarker.current !== null){
            //         beforeMarker.current.setAnimation(null);
            //     }
            ...
            //mapDetail 타입에서 클릭이벤트 처리
            if(props.mapConf.fnType === 'mapDetail'){
                for(let i=0; i<markerMinMaxArray.current.length; i++){
                    if(markerMinMaxArray.current[i].nodeId === props.clickData.beginNodeId){
                        markerMinMaxArray.current[i].setIcon(pointImage);
                        startNodeMarker.current = markerMinMaxArray.current[i];
                    }else if(markerMinMaxArray.current[i].nodeId === props.clickData.endNodeId){
                        markerMinMaxArray.current[i].setIcon(pointImage);
                        endNodeMarker.current = markerMinMaxArray.current[i];
                    }
                }
            }
        }
    ...
    useEffect(() => {
        ...
        if(mode === "C"){
            ...
            //출도착 마커 초기화
            startNodeMarker.current = null;
            endNodeMarker.current = null;
            ...
            for(let i=0; i<polyLineMinMaxArray.current.length; i++){
                navermaps.Event.addListener(polyLineMinMaxArray.current[i], 'click', function (e) {
                    ...
                    if(e.overlay.getOptions("strokeColor") === "#808080"){
                        ...
                        e.overlay.setMap(map.current);
                        polyLineArray.current.push(e.overlay);
                        console.log(polyLineArray.current);
                        console.log(polyLineMinMaxArray.current);
                    }else{
                        e.overlay.setOptions("strokeColor", "#808080");
                    ...
                    if(e.overlay.getIcon() === null){
                        if(document.activeElement.id === "beginNodeId"){
                            ...
                            if(startNodeMarker.current === null){
                                ...
                                //기존 시작노드 초기화
                                for(let i=0; i<markerMinMaxArray.current.length; i++){
                                    if(startNodeMarker.current.nodeId === markerMinMaxArray.current[i].nodeId){
                                        markerMinMaxArray.current[i].setIcon(null);
                                    }
                                }
                            ...
                            if(endNodeMarker.current === null){
                                ...
                                //기존 종료노드 초기화
                                for(let i=0; i<markerMinMaxArray.current.length; i++){
                                    if(endNodeMarker.current.nodeId === markerMinMaxArray.current[i].nodeId){
                                        markerMinMaxArray.current[i].setIcon(null);
                                    }
                                }
